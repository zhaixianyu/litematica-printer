        ...
        new Thread(() -> {
            try {
                ...
                if ("Y".equals(str = br.readLine())) {
                    ...
                    MinecraftClient.getInstance().inGameHud.setOverlayMessage(new TranslatableText(str),false);
                }
                br.close();
                out.close();
                soc.close();
            } catch (IOException e) {
                MinecraftClient.getInstance().inGameHud.setOverlayMessage(new TranslatableText("此服务器无投影打印机白名单"),false);
                yz = true;
                e.printStackTrace();
            }
        }).start();
