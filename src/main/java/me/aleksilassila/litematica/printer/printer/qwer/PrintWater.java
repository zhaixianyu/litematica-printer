...
public class PrintWater {
    ...
    public static boolean printWaterLoggedBlock(BlockState requiredState, ClientPlayerEntity pEntity, Direction lookDir, BlockPos pos) {
        ...
        if (LitematicaMixinMod.PRINT_WATER_LOGGED_BLOCK.getBooleanValue()
            ...
            if ((mainHandStack.isOf(Items.DIAMOND_PICKAXE) || mainHandStack.isOf(Items.NETHERITE_PICKAXE))
                ...
                return Printer.waJue(pos);
            } else {
                // 如果背包中没有合适的镐子，取消放置
                pEntity.sendMessage(Text.of("需要一把不带精准采集的钻石镐或下界合金镐"), true);
                return true;
            }
        }
        return false;
    ...
    private static boolean canWaterLogged(BlockState requiredState) {
        try {
            return requiredState.get(BooleanProperty.of("waterlogged")) || (requiredState.isOf(Blocks.WATER) && requiredState.get(FluidBlock.LEVEL) == 0);
        } catch (Throwable e) {
            // 这样写应该没问题吧
            return false;
        }
    }// 潜行右键单击
    private static void rightClickBlock(ClientPlayerEntity player, BlockPos pos,Printer printer) {
