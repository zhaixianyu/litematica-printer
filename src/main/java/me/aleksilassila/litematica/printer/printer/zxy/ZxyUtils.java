    ...
    public static void addInv() {
        if (adding && !openIng) {
            if (invBlockList.size() == 0) {
                adding = false;
                client.inGameHud.setOverlayMessage(new TranslatableText("打印机库存添加完成"), false);
                return;
            }
            client.inGameHud.setOverlayMessage(new TranslatableText("添加库存中"), false);
                ...
                if (LitematicaMixinMod.SYNC_INVENTORY.getKeybind().isKeybindHeld() && client.crosshairTarget != null && client.crosshairTarget.getType() == HitResult.Type.BLOCK) {
                    ...
                    if (client.world != null) {
                        ...
                        try {
                            ...
                            client.inGameHud.setOverlayMessage(new TranslatableText("八嘎，这不是容器 无法同步"), false);
                            return;
                        }
                    }
...
//                                    System.out.println(i+"  currNum\t"+currNum);
                    ...
                    client.inGameHud.setOverlayMessage(new TranslatableText("剩余 "+syncPosList.size()+" 个容器. 再次按下快捷键取消同步"), false);
