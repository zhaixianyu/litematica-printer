                ...
                if (sc.slots.get(y).getStack().getItem().equals(item)) {
                    ...
                    for (String s : str) {
                        ...
                        try {
                            ...
                            if (Registry.ITEM.getId(player.getInventory().getStack(c).getItem()).toString().contains("shulker_box") && LitematicaMixinMod.QUICKSHULKER.getBooleanValue()) {
                                MinecraftClient.getInstance().inGameHud.setOverlayMessage(new TranslatableText("无可替换的槽位，请将预选位的濳影盒换个位置"),false);
                                continue;
                            }
