...
import static me.aleksilassila.litematica.printer.printer.zxy.OpenInventoryPacket.pos;
    ...
    public static boolean areStacksEquivalent(@NotNull ItemStack stack1, @NotNull ItemStack stack2, boolean ignoreNbt) {
        return stack1.getItem() == stack2.getItem() && (ignoreNbt || !stack1.hasNbt() && !stack2.hasNbt() || Objects.equals(stack1.getNbt(), stack2.getNbt()))
                ||
                    ...
                    return me.aleksilassila.litematica.printer.printer.memory.MemoryUtils.areStacksEquivalent(stack1, candidate, stack1.getNbt() == null);
                });
...
//                System.out.println("latestPos "+latestPos +"   "+ key);
            if (database != null && latestPos != null && key != null) {
                List<ItemStack> stacks = condenseItems(screen.getScreenHandler().slots.stream().filter(me.aleksilassila.litematica.printer.printer.memory.MemoryUtils::isValidSlot).map(Slot::getStack).collect(Collectors.toList()));
